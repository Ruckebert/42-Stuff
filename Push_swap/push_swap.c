...
/*   Updated: 2024/08/20 12:08:24 by aruckenb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**convert_to_indexes(char **number, int argc)
{
	...
	i = 0;
	indexes = ft_calloc(sizeof(char *), argc + 1);
	if (!indexes)
		return (NULL);
	temp = ft_calloc(sizeof(char *), argc + 1);
	if (!temp)
	...
	while (i <= argc - 1)
	{
		...
		if (!indexes[i])
			return (reverse_free(indexes, i), reverse_free(temp, i), NULL);
		temp[i] = ft_itoa(i + 1);
		if (!temp[i])
