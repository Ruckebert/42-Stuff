...
/*   Updated: 2024/08/26 16:24:55 by aruckenb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	image_creation2(t_vars *vars)
{
	vars->images = mlx_xpm_file_to_image(vars->mlx, "Assets/Egg1.xpm",
			&vars->img_width, &vars->img_height);
	if (!vars->images)
		return (image_error(vars), 1);
	print_consumable(vars->map, vars);
	mlx_destroy_image(vars->mlx, vars->images);
	vars->images = mlx_xpm_file_to_image(vars->mlx, "Assets/Exit.xpm",
			&vars->img_width, &vars->img_height);
	if (!vars->images)
		return (image_error(vars), 1);
...
{
	vars->win = mlx_new_window(vars->mlx, ((ft_strlen(vars->map[0]) - 1) * 36),
			((vars->map_size - 1) * 36), "Hello Sperm World!");
	if (!vars->win)
		return (free(vars->mlx), 1);
	vars->images = mlx_xpm_file_to_image(vars->mlx, "Assets/Walls.xpm",
			&vars->img_width, &vars->img_height);
	if (!vars->images)
		return (image_error(vars), 1);
