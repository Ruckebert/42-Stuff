...
void fatal_error()
{
	write(2, "fatal error has occured\n", 25);
	exit(1);
}

void noticify_others(int author, char *str)
{
	int fd = 0;
	while (fd <= max_fd)
	{
...
void register_client(int fd)
{
	...
	noticify_others(fd, buf_write);
}

void remove_client(int fd)
{
	sprintf(buf_write, "bai bai loser %d", ids[fd]);
	noticify_others(fd, buf_write);
	free(msgs[fd]);
	FD_CLR(fd, &afds);
	close(fd);
