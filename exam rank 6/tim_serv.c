...
fd_set rfds, wfds, current;
char send_buffer[40000], recv_buffer[1024];
int maxfd, gid = 0;
...
void	send_all(int fd_sender)
{
	...
	while (fd <= maxfd)
	{
		if (FD_ISSET(fd, &wfds) && fd != fd_sender)
		{
			...
			if (FD_ISSET(fd, &rfds))
			{
				...
				else 
				{
					...
					else 
					{
						...
						{
							char *message = NULL;
							int reval = extract_message(&clients[fd].buffer, &message);
							if (reval == -1)
								close(sockfd), fatal_error();
							if (reval == 0)
								break ;
							sprintf(send_buffer, "clients %d: %s", clients[fd].id, message);
							send_all(fd);
							free(message);
						}
